...
def loadImg(path):
    ...
    for i in files:
        ...
        if os.path.splitext(i)[1].lower() == '.png' and os.path.splitext(i)[0].lower() != 'hierarchicalresult':
...
resultFile = loadImg(r"C:\Users\lin\Pictures\AndroidMaterial\48x48")  # 读取数据，并返回最后结果要存储的文件名，目录根据自己存的位置进行修改
if resultFile != 'None':
    run()
